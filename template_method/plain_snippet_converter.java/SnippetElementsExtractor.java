...
class SnippetElementsExtractor {
  ...
  private ArrayList<HTMLSnippetElementAbstractClass> extractedElements;

  public SnippetElementsExtractor(File snippetFile) {
    this.snippetFile = snippetFile;
    extractedElements = new ArrayList<HTMLSnippetElementAbstractClass>();
  }

  public ArrayList<HTMLSnippetElementAbstractClass> extract() throws ParserConfigurationException, SAXException, IOException {
  ...
  private void constructElement(Node element) {
    HTMLSnippetElementAbstractClass HTMLSnippetElement;

    switch (element.getNodeName()) {
      case "paragraph":
        String refinedParagraph = refineText(element.getTextContent());
        HTMLSnippetElement = new HTMLSnippetPlainElementConcreteClass("p", refinedParagraph);
      ...
      case "list":
        ...
        HTMLSnippetElement = new HTMLSnippetNestingElementConcreteClass("ul", "li", listItemsContents);
        extractedElements.add(HTMLSnippetElement);

        break;
      case "source":
        String refinedSource = refineText(element.getTextContent());
        HTMLSnippetElement = new HTMLSnippetPlainElementConcreteClass("cite", refinedSource);
        extractedElements.add(HTMLSnippetElement);
