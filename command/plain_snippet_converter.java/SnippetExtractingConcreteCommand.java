...
class SnippetExtractingConcreteCommand implements SnippetConvertingCommand {
  private String snippetInputFilePath;
  private File snippetFile;
  private ArrayList<String> extractedElements;
  private SnippetPersistingReceiver receiver;

  public SnippetExtractingConcreteCommand(String snippetInputFilePath, SnippetPersistingReceiver receiver) {
  ...
  public void execute() throws ParserConfigurationException, SAXException, IOException {
    snippetFile = new File(snippetInputFilePath);
    extract();
    String extractedContent = String.join("", extractedElements);

    receiver.action(extractedContent);
  }
